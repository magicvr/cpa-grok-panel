...
flowchart LR
  ...
  APP --> DOMAIN[领域模型与规则]
  APP --> PORTS[端口接口]
  PORTS --> AUTH[Management Auth REST 适配器]
...
- 插件注册、能力声明、路由与静态资源挂载。
- 配置重载和版本/能力协商。
- usage 事件校验、归一化、精确/弱去重键提取。
- Management auth REST 请求和宿主错误映射。
- 管理请求的宿主认证上下文提取。
...
领域层只接收内部 `AuthIndex` 与写入所需的 `ExactFileName`，不知道 CPA JSON、HTTP 或磁盘格式。
...
- 状态仓储：原子快照、迁移、备份、锁。
- CPA auth 网关：通过 Management REST 列出、更新优先级、启停、删除。
...
2. 启动同步：调用 Management `GET /v0/management/auth-files` 建立 `auth_index`→`name` 账号投影；不触发外部健康/套餐检查。
...
1. CPA 通过插件 host 调用 `usage.handle`；事件必须能提取可信 `auth_index`，否则只计未归因指标。
2. 适配层校验版本、可选事件 id、token 非负性并规范化。
3. 应用服务在有稳定 `event_id` 时精确去重；没有时按 `weak_dedupe` 策略抑制短窗明显重复，再交给领域模型累计真实 token。
4. 成功事件清零失败序列；失败事件按设置累计。
5. 达到阈值时，领域产生 `PriorityDemotionRequested`；usage 热路径只原子记账并把唯一降权意图送入有界异步队列。
6. 降权 worker 重新 list，以 `auth_index` 解析当前 `exact_file_name` 和 priority；保存基线后串行调用 `PATCH /v0/management/auth-files/fields`，写入配置的 `demotion_priority`。
7. worker 写后再次 list 校验目标 name 与最终 priority。成功则更新投影和审计；失败/结果不确定则保持待重试或失败状态，不阻塞 handler，也不自行修改 auth 文件。

降权决策绝不使用 `provider` 字符串进行账号映射。没有可信 `auth_index` 的失败事件只进入“未归因事件”指标。失败归责采用版本化 allowlist：401/403 默认可归责，429/5xx 是否纳入由配置和策略版本决定；不存在正式 attribution 枚举时不得虚构。

### 4.2 管理面板读取

1. 浏览器请求插件管理路由，由 CPA 管理鉴权保护。
2. API 查询 host auth 清单和插件状态快照。
3. 应用服务按 `auth_index` 合并为只读 AccountView，并保留当前精确 `.json` 文件名供未来 M2/M3 写操作使用。
...
3. 仅当 CPA 提供能锁定指定 auth 的 `host.auth.invoke` 或等价正式安全能力时，worker pool 才按设置限流发起检查。
4. 每个账号独立记录成功、失败、超时或取消；结果写入 state。
5. UI 轮询操作端点直到终态，刷新账号视图。

### 4.4 安全删除

1. UI 先请求删除预检，提交 `auth_index` 与 `exact_file_name`。
2. 服务重新读取 Management auth 清单，确认双键仍映射到同一条目，执行保护规则，返回短期确认令牌和风险摘要。
3. 管理员再次提交相同身份、精确文件名、确认令牌和确认文本。
4. 服务再次校验身份未变化、令牌未过期、保护规则仍通过。
5. 仅调用精确 name 的 Management DELETE；随后 re-list 复核。成功后保留墓碑和审计，不立即抹除历史统计。

## 5. 一致性模型

- CPA auth 元数据为强真相源，插件 AccountView 是短时投影。
- usage 计数以插件回调实际投递为准：稳定 `event_id` 下可实现效果上的一次累计；`weak_dedupe` 只降低重复概率，不提供账本级保证。
- 批量操作不是跨账号事务；返回逐项结果。
- 当前 Management 列表无 revision。单账号 auth 写由内部队列串行化，写前/写后 re-list、防抖与目标值校验降低竞态，并明确接受 last-write-wins；若未来宿主提供 revision，`expected_revision` 作为可选增强启用。
- state 内单账号更新和事件去重记录应在同一持久化提交中完成。

## 6. 可观测性

- 日志字段：`request_id`、`operation_id`、哈希化 `auth_index`、动作、结果、错误码、耗时。
...
- 不能在 CPA 未暴露 revision/条件写时声称拥有 CAS；只能执行已冻结的串行、校验与 LWW 策略。
