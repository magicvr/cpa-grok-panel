...
`cpa-grok-panel` 是 CLIProxyAPI（CPA）的 Grok/xAI OAuth 账号运维面板。v0.3.0 已提供可安装的 Linux amd64 原生插件，用于查看账号与真实用量、执行启停和优先级操作、自动降权、安全删除，以及按日清零统计。
...
1. 打开 [GitHub Releases](https://github.com/magicvr/cpa-grok-panel/releases)，下载 `cpa-grok-panel_0.3.0_linux_amd64.zip`。
...
- 支持 20、50、100 条分页，并可按账号文件、状态、优先级、总 Token、成功数或失败数排序。
...
架构、CPA 集成、接口和持久化等设计资料仍保留在 [docs/design/](docs/design/)；README 以当前 v0.3.0 的可安装版本为准。
